...
from keyboards.main_menu import get_main_keyboard, get_faq_keyboard, get_exercises_keyboard, get_payment_keyboard, get_nutrition_keyboard, get_progress_keyboard
...
async def open_training(message: Message):
    ...
    if not has_access:
        await message.answer("Раздел «Тренировка» доступен только с активным абонементом.")
        return
    await message.answer("Выбери опцию в разделе «Тренировка»:", reply_markup=get_training_keyboard())
