    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        await db.execute('''
            CREATE TABLE IF NOT EXISTS payments (
                ...
                product_type TEXT DEFAULT 'full',
        ...
        await db.execute('''
            CREATE TABLE IF NOT EXISTS favorite_recipes (
                ...
                UNIQUE(user_id, recipe_category, recipe_id),
                FOREIGN KEY (user_id) REFERENCES users (user_id)
            )
        ''')
        await db.execute('''
...
async def update_user_access(user_id: int, days_to_add: int = 30):
    """Добавляет дни доступа к существующей дате или устанавливает новую"""
    async with aiosqlite.connect(DB_PATH) as db:
        # Получаем текущую дату доступа
        async with db.execute(
            "SELECT access_until FROM users WHERE user_id = ?",
            (user_id,)
        ) as cursor:
        ...
        if result and result[0]:
            try:
                # Если есть существующая дата, добавляем дни к ней
                current_date = datetime.datetime.strptime(result[0], '%Y-%m-%d').date()
                new_access_date = current_date + datetime.timedelta(days=days_to_add)
            except ValueError:
                # Если дата в неправильном формате, создаем новую от сегодня
                new_access_date = datetime.date.today() + datetime.timedelta(days=days_to_add)
        else:
            # Если даты нет, создаем новую от сегодня
            new_access_date = datetime.date.today() + datetime.timedelta(days=days_to_add)
...
async def add_payment(user_id, screenshot_id, product_type='full', days=30):
    date_now = datetime.datetime.now().isoformat()
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute(
            "INSERT INTO payments (user_id, screenshot_id, product_type, days, status, date) VALUES (?, ?, ?, ?, 'pending', ?)",
            (user_id, screenshot_id, product_type, days, date_now)
        )
        await db.commit()
...
async def get_pending_payments():
    async with aiosqlite.connect(DB_PATH) as db:
        async with db.execute(
            """SELECT p.user_id, u.username, u.full_name, p.screenshot_id, p.date, p.days 
               FROM payments p 
...
async def update_payments_table():
    """Добавляет колонку days в таблицу payments если ее нет"""
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        if 'days' not in column_names:
            await db.execute("ALTER TABLE payments ADD COLUMN days INTEGER DEFAULT 30")
            await db.commit()
            print("✅ Колонка 'days' добавлена в таблицу payments")

# Функции для избранных рецептов
async def add_favorite_recipe(user_id: int, recipe_category: str, recipe_id: int):
    """Добавляет рецепт в избранное"""
    async with aiosqlite.connect(DB_PATH) as db:
        try:
            await db.execute(
                "INSERT OR IGNORE INTO favorite_recipes (user_id, recipe_category, recipe_id) VALUES (?, ?, ?)",
                (user_id, recipe_category, recipe_id)
            )
            await db.commit()
            return True
...
async def remove_favorite_recipe(user_id: int, recipe_category: str, recipe_id: int):
    """Удаляет рецепт из избранного"""
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute(
            "DELETE FROM favorite_recipes WHERE user_id = ? AND recipe_category = ? AND recipe_id = ?",
            (user_id, recipe_category, recipe_id)
        )
        await db.commit()
...
async def get_favorite_recipes(user_id: int):
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        async with db.execute(
            "SELECT recipe_category, recipe_id FROM favorite_recipes WHERE user_id = ? ORDER BY added_date DESC",
            (user_id,)
        ) as cursor:
            results = await cursor.fetchall()
            return results

async def is_recipe_favorite(user_id: int, recipe_category: str, recipe_id: int):
    """Проверяет, есть ли рецепт в избранном"""
    async with aiosqlite.connect(DB_PATH) as db:
        async with db.execute(
            "SELECT 1 FROM favorite_recipes WHERE user_id = ? AND recipe_category = ? AND recipe_id = ?",
            (user_id, recipe_category, recipe_id)
        ) as cursor:
            result = await cursor.fetchone()
