...
from database import update_onboarding_data, get_full_onboarding_data
...
async def process_experience(callback: CallbackQuery, state: FSMContext):
    exp_map = {
        ...
        "exp_beginner": "Начинающий (0-6 месяцев)",
    ...
    await state.set_state(QuestionnaireState.waiting_for_equipment)
...
async def process_equipment(callback: CallbackQuery, state: FSMContext):
    ...
    await state.set_state(QuestionnaireState.waiting_for_schedule)
    await callback.answer()
...
async def process_schedule(callback: CallbackQuery, state: FSMContext):
    ...
    await state.set_state(QuestionnaireState.waiting_for_food_preferences)
    await callback.answer()
...
async def process_weight(message: Message, state: FSMContext):
    try:
        ...
        if 30 <= weight <= 200:
            ...
            await state.set_state(QuestionnaireState.waiting_for_activity_level)
        else:
